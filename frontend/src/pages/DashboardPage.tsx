...
interface Widget {
  ...
  label: string;
  description: string;
  roles: UserRole[]; // Which roles can see this widget
}
...
const AVAILABLE_WIDGETS: Widget[] = [
  {
    ...
    roles: ['foreman', 'superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'todays_tasks',
    label: "Today's Tasks",
    description: 'Tasks assigned to you',
    roles: ['field_worker', 'foreman', 'superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'manpower_today',
    label: "Today's Crew",
    description: 'Expected manpower for today',
    roles: ['foreman', 'superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'upcoming_deliveries',
    label: 'Upcoming Deliveries',
    description: 'Deliveries arriving soon',
    roles: ['foreman', 'superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'late_deliveries',
    label: 'Late Deliveries',
    description: 'Overdue deliveries needing attention',
    roles: ['superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'pending_reviews',
    label: 'Pending Reviews',
    description: 'Field submissions awaiting review',
    roles: ['project_manager', 'admin'],
  },
  {
    id: 'recent_reports',
    label: 'Recent Field Reports',
    description: 'Latest daily reports from the field',
    roles: ['superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'project_status',
  ...
  {
    id: 'look_ahead_summary',
    label: '2-Week Look Ahead',
    description: 'Upcoming work summary',
    roles: ['superintendent', 'project_manager', 'admin'],
  },
  {
    id: 'my_hours',
    label: 'My Hours',
    description: 'Your time this week',
...
export function DashboardPage() {
  ...
  const getAvailableWidgets = () => {
    return AVAILABLE_WIDGETS.filter(w => w.roles.includes(userRole));
  };
          ...
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              <button
                ...
                className="flex flex-col items-center gap-1 p-3 bg-purple-50 rounded-xl"
              >
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/tasks')} className="text-blue-600 text-sm">
                View All
              </button>
            </div>
            {tasks.length === 0 ? (
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/look-ahead')} className="text-blue-600 text-sm">
                Plan
              </button>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-3xl font-bold text-gray-900">
                  {todayLookAhead?.manpower || '—'}
                </div>
                <div className="text-sm text-gray-500">Expected workers</div>
              </div>
            </div>
            {todayLookAhead?.plannedWork && todayLookAhead.plannedWork.length > 0 && (
              <div className="mt-3 pt-3 border-t">
                <div className="text-xs text-gray-500 mb-1">Planned Work</div>
                <div className="flex flex-wrap gap-1">
                  {todayLookAhead.plannedWork.slice(0, 3).map((work, i) => (
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/deliveries')} className="text-blue-600 text-sm">
                View All
              </button>
            </div>
            {upcoming.length === 0 ? (
                ...
                {upcoming.map(delivery => (
                  <div key={delivery.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ml-2 ${
                      delivery.estimatedArrival < new Date().toISOString().split('T')[0]
      ...
      case 'late_deliveries':
        ...
        return (
          <div key={widgetId} className="bg-red-50 rounded-xl border border-red-200 p-4">
            ...
            <button
              onClick={() => navigate('/deliveries')}
              className="w-full mt-3 py-2 text-sm text-red-700 font-medium"
            >
              Manage Deliveries →
            </button>
          </div>
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => navigate('/pm-queue')}
                className="p-3 bg-amber-50 rounded-xl text-left"
              >
              ...
              <button
                ...
                className="p-3 bg-green-50 rounded-xl text-left"
              >
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/pm-dashboard')} className="text-blue-600 text-sm">
                View All
              </button>
            </div>
            {reports.length === 0 ? (
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/projects')} className="text-blue-600 text-sm">
                View All
              </button>
            </div>
            {projects.length === 0 ? (
      ...
      case 'look_ahead_summary':
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <button
              onClick={() => navigate('/look-ahead')}
              className="w-full flex items-center gap-4 p-3 bg-purple-50 rounded-xl"
            >
        ...
        return (
          <div key={widgetId} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/timecard')} className="text-blue-600 text-sm">
                Log Time
              </button>
            </div>
            <div className="flex items-center gap-4">
  ...
  // Field worker gets a simple fixed view (not customizable)
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="p-4 space-y-4">
          {/* Quick Actions for Field Worker */}
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => navigate('/capture/photo')}
              className="bg-white rounded-xl p-4 border border-gray-200 flex flex-col items-center gap-2"
            >
  ...
  // Customizable dashboard for other roles
  return (
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <header className="bg-blue-600 text-white px-4 py-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button 
              ...
              className={`p-2 rounded-lg ${isCustomizing ? 'bg-white/20' : 'hover:bg-white/10'}`}
            >
              <Settings size={20} />
            </button>
          </div>
        ...
        <div className="text-blue-100 text-sm capitalize">
          {user?.firstName} · {userRole.replace('_', ' ')}
        </div>
      </header>
